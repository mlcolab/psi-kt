...
def read_data(input_file, method, omit_bias=False, omit_lexemes=False, max_lines=None):
    ...
    for i, row in enumerate(reader):
        ...
        t = float(row['delta'])/(60*60*24)  # convert time delta to days # time (in seconds) since the last lesson/practice that included this word/lexeme
        h = hclip(-t/(math.log(p, 2))) # half_life because p = 2**(-t/h)
