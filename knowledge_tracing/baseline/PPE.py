...
class PPE(BaseLearnerModel):
    ...
    def simulate_path(self, x0, t, items=None, stats=None, user_id=None, stats_cal_on_fly=False): 
        ...
        for i in range(1, time_step):
            ...
            lags = torch.diff(cur_history_last_time) / T_SCALE + EPS # [bs, num_node, i]
            lag_mask = (lags > 0)
            dn = ((1 / torch.log(abs(lags + EPS) + np.e)) * lag_mask).sum(dim=-1) / (cur_repeat + 1 + EPS) # [bs, num_node]
            dn = batch_b + batch_m * dn.unsqueeze(-1) # [bs, num_node, 1]

            # - big T
            small_t = (t[..., i:i+1] - whole_last_time[..., :i+1])/T_SCALE + EPS
            ...
            big_t = torch.nan_to_num(torch.pow(small_t+EPS, batch_x)) / \
                        (torch.sum(torch.nan_to_num(torch.pow(small_t+EPS, batch_x)), 1, keepdims=True) + EPS)
            big_t = torch.sum(big_t * small_t, -1)[..., None] # [bs, num_node]
            big_t = torch.nan_to_num(big_t)
            
            big_t_mask = (big_t!=0)
            mn = torch.nan_to_num(torch.pow((whole_stats[:,:,i:i+1,0]+1), batch_lr)) * \
                        torch.nan_to_num(torch.pow((big_t + EPS), -dn)) * big_t_mask
            mn = torch.nan_to_num(mn)
            
            pn = 1/(1 + torch.nan_to_num(torch.exp((batch_tau - mn)/(batch_s + EPS) + EPS) + EPS))
