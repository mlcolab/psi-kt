...
class KTM(BaseModel):
    ...
    def forward(self, feed_dict):
        ...
        if self.emb_size > 0:
            ...
            predictions += 0.5 * (
                interaction.sum(-2) ** 2 - (interaction**2).sum(-2)
            ).sum(-1)
    ...
    def loss(self, feed_dict, outdict):
        ...
        predictions, labels, lengths = (
            outdict["prediction"][indice],
            outdict["label"][indice],
            lengths[indice],
        )
        predictions = torch.nn.utils.rnn.pack_padded_sequence(
            predictions, lengths, batch_first=True
        ).data
        labels = torch.nn.utils.rnn.pack_padded_sequence(
            labels, lengths, batch_first=True
        ).data
    ...
    def get_feed_dict(self, corpus, data, batch_start, batch_size, phase):
        ...
        feed_dict = {
            "idx": torch.from_numpy(
                fm_idxs
            ).long(),  # [batch_size, real_max_step, max_num_idx]
            "val": torch.from_numpy(
                fm_vals
            ),  # [batch_size, real_max_step, max_num_idx]
            "length": torch.from_numpy(lengths),  # [batch_size]
            "label_seq": torch.from_numpy(utils.pad_lst(label_seqs)),  # [batch_size]
        }
        return feed_dict

    def construct_sparse_feature(
        self, user_seqs, label_seqs, problem_seqs, time_seqs, lengths
    ):
        real_batch_size = user_seqs.shape[0]
        t1_seq, t2_seq, t3_seq = DKTForgetting.get_time_features(user_seqs, time_seqs)

        max_step, max_num_idx = (
            np.max(lengths),
            2 + min(self.max_step - 1, self.skill_num * 2) + 4,
        )
        fm_idxs, fm_vals = np.zeros((real_batch_size, max_step, max_num_idx)), np.zeros(
            (real_batch_size, max_step, max_num_idx)
        )
        real_num_idx = 0
