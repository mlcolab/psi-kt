...
class SKT(BaseModel):
    ...
    def __init__(
        ...
        self.graph = Graph.from_file(
            self.skill_num, graph_path
        )  # TODO check graph_params
    ...
    def forward(self, feed_dict, states=None):  # TODO check load model states
        ...
        if states is None:  # TODO
            states = self.begin_states(
                shapes=[(batch_size, self.skill_num, self.hidden_num)],
                prefix=self.prefix,
            )[0]
        ...
        for i in range(length - 1):
            ...
            next_states, _ = self.rnn(
                torch.cat((_inf, concept_embeddings), dim=-1), states
            )
            states = next_states
