...
class Graph(object):
    ...
    def neighbors(
        self, x, ordinal=True, merge_to_one=True, with_weight=False, excluded=None
    ):
        ...
        if isinstance(x, list):
            ...
            if not ordinal:
                if len(self.undirected_graphs) == 0:
                    ...
                    return [
                        v
                        for v in self.undirected_graphs[0].neighbors(int(x))
                        if v not in excluded
                    ]
                else:
                    if not merge_to_one:
                        return [
                            [v for v in graph.neighbors(int(x)) if v not in excluded]
                        ...
                        for graph in self.undirected_graphs:
                            _ret.extend(
                                [
                                    v
                                    for v in graph.neighbors(int(x))
                                    if v not in excluded
                                ]
                            )
                        return _ret
        ...
        if isinstance(x, list):
            ...
            if not ordinal:
                if len(self.directed_graphs) == 0:
                    ...
                    return [
                        v
                        for v in self.directed_graphs[0].successors(int(x))
                        if v not in excluded
                    ]
                else:
                    if not merge_to_one:
                        return [
                            [v for v in graph.successors(int(x)) if v not in excluded]
                        ...
                        for graph in self.directed_graphs:
                            _ret.extend(
                                [
                                    v
                                    for v in graph.successors(int(x))
                                    if v not in excluded
                                ]
                            )
                        return _ret
...
class SKTF1(BaseModel):
    ...
    def parse_model_args(parser, model_name="SKTF1"):
        parser.add_argument(
            "--graph_params",
            default=[
                ["correct_transition_graph.json", True],
                ["transition_graph.json", False],
            ],
        )
    ...
    def __init__(
        self, args, corpus
    ):  # TODO super(SKT, self).__init__(prefix=prefix, params=params)
        self.dataset = args.dataset
        self.problem_num = int(corpus.n_problems)
        self.skill_num = int(corpus.n_skills)  # =self.ku_num
        self.hidden_num = (
            self.skill_num if args.hidden_num is None else int(args.hidden_num)
        )
        self.latent_dim = (
            self.hidden_num if args.latent_dim is None else int(args.latent_dim)
        )
        self.concept_dim = (
            self.hidden_num if args.concept_dim is None else int(args.concept_dim)
        )
        ...
        for graph in graph_params:
            graph_path.append([])
            graph_path[-1].append(
                os.path.join(args.path, args.dataset, "graph", graph[0])
            )
        ...
        self.sync_activation = (
            args.sync_activation if args.activation is None else args.activation
        )
        self.prop_activation = (
            args.prop_activation if args.activation is None else args.activation
        )
        self.agg_activation = (
            args.agg_activation if args.activation is None else args.activation
        )
    ...
    def _init_weights(self):
        ...
        self.response_embedding = torch.nn.Embedding(
            2 * self.skill_num, self.latent_dim
        )
        self.concept_embedding = torch.nn.Embedding(
            self.skill_num, self.concept_dim
        )  # TODO no gradient
        self.f_self = torch.nn.GRUCell(self.hidden_num, self.hidden_num).to(
            torch.float64
        )
        ...
        self.out = torch.nn.Linear(self.hidden_num, 1).to(
            torch.float64
        )  # TODO loss function
    ...
    def begin_states(
        self, shapes, prefix, func=torch.zeros
    ):  # func=mx.nd.zeros # in rnn.py
        states = []
    ...
    def forward(self, feed_dict):
        ...
        states = self.begin_states(
            shapes=[(batch_size, self.skill_num, self.hidden_num)], prefix=self.prefix
        )[0]
        ...
        for i in range(length - 1):
            ...
            fin = self.fin(
                torch.cat(
                    (
                        repeated_time_gap_seq[:, i],
                        sequence_time_gap_seq[:, i],
                        past_trial_counts_seq[:, i],
                    ),
                    dim=-1,
                )
            )
            ...
            _next_self_state = self.f_self(
                self.response_embedding(answer_index), _self_state
            )  # TODO test answer_index
            ...
            _self_mask = torch.broadcast_to(
                _self_mask, (batch_size, self.skill_num, self.hidden_num)
            )
            ...
            _neighbors_mask = torch.broadcast_to(
                _neighbors_mask, (batch_size, self.skill_num, self.hidden_num)
            )
            ...
            _broadcast_next_self_states = torch.broadcast_to(
                ...
                (batch_size, self.skill_num, self.hidden_num),
            )
            ...
            _reflec_inf = torch.broadcast_to(
                _reflec_inf.unsqueeze(1), (batch_size, self.skill_num, self.hidden_num)
            )
            ...
            _successors_mask = torch.broadcast_to(
                _successors_mask, (batch_size, self.skill_num, self.hidden_num)
            )
            ...
            _prop_inf = _successors_mask * torch.broadcast_to(
                _prop_inf.unsqueeze(1), (batch_size, self.skill_num, self.hidden_num)
            )
            # concept embedding
            concept_embeddings = self.concept_embedding.weight.data
            concept_embeddings = torch.broadcast_to(
                concept_embeddings.unsqueeze(0), _prop_inf.shape
            )

            # aggregate
            fin = torch.broadcast_to(fin.unsqueeze(1), states.shape)
            _inf = self.f_agg(self.alpha * _sync_inf + (1 - self.alpha) * _prop_inf)
            next_states, _ = self.rnn(
                torch.cat((_inf, concept_embeddings, fin), dim=-1), states
            )
            states = next_states

            output = torch.sigmoid(
                torch.squeeze(self.out(self.dropout(states)), dim=-1)
            )

            next_skill = skills[:, i + 1]
            next_index = torch.cat(
                (
                    torch.arange(0, output.shape[0]).unsqueeze(-1),
                    next_skill.unsqueeze(-1),
                ),
                dim=-1,
            )
        ...
        out_dict = {"prediction": outputs, "label": labels[:, 1:].double()}
        return out_dict
    ...
    def get_feed_dict(self, corpus, data, batch_start, batch_size, phase):
        ...
        feed_dict = {
            ...
            "repeated_time_gap_seq": torch.from_numpy(
                repeated_time_gap_seq[indice]
            ),  # [batch_size, max_step]
            "sequence_time_gap_seq": torch.from_numpy(
                sequence_time_gap_seq[indice]
            ),  # [batch_size, max_step]
            "past_trial_counts_seq": torch.from_numpy(
                past_trial_counts_seq[indice]
            ),  # [batch_size, max_step]
