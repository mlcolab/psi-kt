...
def get_gkt_graph(
    num_c, dpath, trainfile, testfile, graph_type="dense", tofile="./graph.npz"
):
    graph = None
...
def build_transition_graph(df, concept_num):
    ...
    for _, row in df.iterrows():
        questions = list(filter(lambda x: x != "-1", row["concepts"].split(",")))
...
def build_dense_graph(concept_num):
    ...
    Returns:
        numpy: graph
    """
    graph = 1.0 / (concept_num - 1) * np.ones((concept_num, concept_num))
    np.fill_diagonal(graph, 0)
    graph = torch.from_numpy(graph).float()
    return graph
