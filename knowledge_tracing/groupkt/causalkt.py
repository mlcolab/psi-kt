...
from knowledge_tracing.groupkt.graph_message_passing import *
...
class CausalKT(BaseModel):
    ...
    def parse_model_args(parser, model_name="CausalKT"):
        ...
        parser.add_argument(
            "--emb_size", type=int, default=2, help="Size of embedding vectors."
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            type=str,
            default="dibs",
            help="[constant, basic, enco, dibs]",
        )
        parser.add_argument(
            "--latent_dim",
            type=int,
            default=32,
            help="embedding vector dimension for dibs",
        )
        parser.add_argument(
            "--num_graph",
            type=int,
            default=1,
            help="the amount of graphs when sampling",
        )
        ...
        parser.add_argument(
            ...
            type=str,
            default="ou",
            help="[target_msg, whole_msg, rnn, perf_trace]",
        )
        ...
        parser.add_argument(
            ...
            type=int,
            default=0,
            help="whether the graph starts from sparse(0) or dense(1)",
        )
        parser.add_argument(
            "--adj_or_prob",
            type=str,
            default="b_adj",
            help=["adj=b*p", "b_adj", "p_adj"],
        )
        parser.add_argument(
            "--diagonal",
            type=int,
            default=1,
            help="for debug use! 0: wo_diagonal; 1: w_diagonal",
        )
        parser.add_argument(
            "--skill_base",
            type=int,
            default=0,
            help="for debug use! 0: wo_problem_base; 1: w_problem_base",
        )
        parser.add_argument(
            "--problem_base",
            type=int,
    ...
    def _init_weights(self):
        ...
        if self.latent_rep == "dibs":
            ...
            self.node_embeddings = torch.cat([u, v], dim=-1).unsqueeze(
                0
            )  # [1, num_nodes, num_emb]
            ...
            node_embeddings = torch.randn(
                1, self.num_nodes, self.emb_size, device=self.device
            )
        ...
        if self.decoder_type == "target_msg":
            self.decoder = GraphTargetMessage(
                self.device, self.num_nodes, self.args, self.emb_size
            )
        elif self.decoder_type == "whole_msg":
            self.decoder = GraphWholeMessage(
                self.device, self.num_nodes, self.args, self.emb_size
            )
        elif self.decoder_type == "rnn":
            self.decoder = GraphRecurrent(
                self.device,
                self.num_nodes,
                self.args,
                self.emb_size,
            ...
            self.decoder = GraphPerformanceTrace(
                self.device, self.num_nodes, self.args, self.emb_size
            )
        elif self.decoder_type == "ou":
            self.decoder = GraphOUProcess(
                self.device, self.num_nodes, self.args, self.emb_size
            )
    ...
    def get_weighted_adjacency(self):
        if self.diagonal == 0:
            W_adj = self.W * (
                1.0 - torch.eye(self.num_nodes, device=self.device)
    ...
    def forward(self, feed_dict):
        ...
        labels = labels[
            :, self.args.time_lag :
        ]  # .unsqueeze(1).tile((1, preds.shape[1], 1)) # TODO in real data should be labels[:, 1:]
