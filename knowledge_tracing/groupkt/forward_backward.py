...
def forward_pass(log_a, log_b, logprob_s0):
    ...
    Args:
        log_a:      a float `Tensor` of size [batch, num_steps, num_categ, num_categ] stores time dependent 
                        transition matrices, `log p(s[t] | s[t-1], x[t-1])`.
                        `A[i, j]` is the transition probability from `s[t-1]=j` to `s[t]=i`.
        log_b:      a float `Tensor` of size [batch, num_steps, num_categ] stores time dependent emission 
                        matrices, 'log p(x[t](, z[t])| s[t])`.
        logprob_s0: a float `Tensor` of size [num_categ], initial discrete states probability, `log p(s[0])`.

    Returns:
        forward_pass: a float 3D `Tensor` of size [batch, num_steps, num_categ] stores the forward pass 
                        probability of `log p(s_t | x_1:t)`, which is normalized.
        normalizer:   a float 2D `Tensor` of size [batch, num_steps] stores the normalizing probability, 
                        `log p(x_t | x_1:t-1)`.
    """
    num_steps = log_a.get_shape().with_rank_at_least(3).dims[1].value

    tas = [torch.TensorArray(torch.float32, num_steps, name=n)
    ...
    init_updates = utils.normalize_logprob(
        logprob_s0[torch.newaxis, :] + log_b[:, 0, :], axis=-1)
    ...
    def _steps(t, prev_prob, fwd_tas):
        ...
        current_updates = torch.math.reduce_logsumexp(
            bi_t[:, :, torch.newaxis] + aij_t + prev_prob[:, torch.newaxis, :],
            axis=-1)
    ...
    forward_prob = torch.transpose(tas_final[0].stack(), [1, 0, 2])
    normalizer = torch.transpose(torch.squeeze(tas_final[1].stack(), axis=[-1]), [1, 0])
...
def backward_pass(log_a, log_b, logprob_s0):
    ...
    """
    batch_size = torch.shape(log_b)[0]
    num_steps = torch.shape(log_b)[1]
    num_categ = logprob_s0.shape[0]

    tas = [torch.TensorArray(torch.float32, num_steps, name=n)
            for n in ["backward_prob", "normalizer"]]

    init_updates = [torch.zeros([batch_size, num_categ]), torch.zeros([batch_size, 1])]
    ...
    def _steps(t, next_prob, bwd_tas):
        ...
        current_updates = torch.math.reduce_logsumexp(
            next_prob[:, :, torch.newaxis] + aij_tp1 + bi_tp1[:, :, torch.newaxis],
            axis=-2)
    ...
    backward_prob = torch.transpose(tas_final[0].stack(), [1, 0, 2])
    normalizer = torch.transpose(torch.squeeze(tas_final[1].stack(), axis=[-1]), [1, 0])
...
def forward_backward(log_a, log_b, log_init):
    ...
    m_fwd = fwd[:, 0:-1, torch.newaxis, :]
    m_bwd = bwd[:, 1:, :, torch.newaxis]
    m_a = log_a[:, 1:, :, :]
    m_b = log_b[:, 1:, :, torch.newaxis]
    ...
    gamma_ij = torch.concat([torch.zeros([torch.shape(log_a)[0], 1,
                                    torch.shape(log_a)[2], torch.shape(log_a)[3]]),
