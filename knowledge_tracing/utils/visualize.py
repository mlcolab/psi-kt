...
def compare_model_task(
    ...
    for i, _ in enumerate(train_sizes):
...
def _create_heatmap(
    ...
    heatmap_labels = pd.DataFrame(
        {"Task": np.repeat(tasks, num_models), "Model": np.tile(model_names, num_tasks)}
    )
    ...
    for i in range(num_models * num_tasks):
        plt.text(
            -1,
            i + 0.5,
            heatmap_labels.iloc[i]["Model"] + ", " + heatmap_labels.iloc[i]["Task"],
            ha="left",
            va="center",
            fontsize=10,
        )
...
def _create_bar(
    ...
    """
    Creates and saves a bar chart for comparing performance metrics of different models across multiple tasks.

    This function generates a figure with a series of subplots, each representing a different task. For each task,
    it plots a bar chart comparing the performance of various models based on specified metrics. The charts include
    custom labels, a legend, and grid lines for clarity. The resulting plot is saved to a file with a given prefix.

    Args:
        data (dict): A nested dictionary where keys are model names, and values are dictionaries mapping task names
                     to lists of metric values. For example, data[model_name][task_name] should return a list of
                     metric values for that model and task.
        tasks (list): A list of strings representing the names of the tasks to be included in the bar charts.
        model_names (list): A list of strings representing the names of the models to be compared.
        metrics (list): A list of strings representing the names of the metrics to be compared.
        save_path (str): The directory path where the resulting plot should be saved.
        prefix (str): A string prefix for the filename of the saved plot.
    """
    num_tasks = len(tasks)
    num_models = len(model_names)
    num_metrics = len(metrics)
...
def compare_model_task_metric(
    data, tasks, model_names, metrics, save_path=None, fig_format="heatmap", prefix=""
):
    """
    Example data:
...
def draw_path(path, args, times, items=None, prefix=None, scatter=False):
    ...
    plt.title(label=process.upper() + " Model", fontsize=20, color="black")

    for i, c in zip(range(args.num_node), color):
        if scatter:
            ind = np.where(items == i)[0]
            plt.plot(
                times[ind], path[ind, i], color=c, label="{}".format(i), linewidth=4
            )
        else:
...
def draw_params(params, args, times, items=None, prefix=None, scatter=False):
    ...
    for keys, values in params.items():
        ...
        plt.title(label=keys, fontsize=20, color="black")

        for i, c in zip(range(args.num_node), color):
            if np.issubdtype(values.dtype, np.integer):
                plt.scatter(
                    times[seq, 1:], values[seq, :, i], color=c, label="{}".format(i)
                )
            else:
                plt.plot(
                    times[seq, 1:], values[seq, :, i], color=c, label="{}".format(i)
...
def visualize_ground_truth(
    graph: nx.Graph, args: argparse.Namespace, adj: np.ndarray, size: int = 5
):
    """
